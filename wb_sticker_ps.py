...
def calculate_sticker_position(png_path):
    """
    计算贴图的关键位置（复刻美图秀秀逻辑）：
    1. 最高有效像素的 y 坐标（alpha >= 20 的最上方像素）
    2. 有效像素的中心点 x 坐标（所有有效像素 x 的平均值）
    """
...
def place_back_new(design_path, torso_path, output_path):
    """
    背图新方案（复刻美图秀秀逻辑）：
    1. 置入贴图，缩放30%，旋转1°
    2. 贴图最高有效像素 → PS Y=570
    ...
    try:
        ...
        scale = config.BACK_NEW["scale_percent"] / 100
